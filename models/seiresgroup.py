...
class iResGroup(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False, norm_layer=None, groups=None,  dropout_prob0=0.0):
        ...
        self.layer1 = self._make_layer(block, 256, layers[0], groups=max(
            1, groups//8), stride=2, norm_layer=norm_layer)
        self.layer2 = self._make_layer(block, 512, layers[1], groups=max(
            1, groups//4), stride=2, norm_layer=norm_layer)
        self.layer3 = self._make_layer(block, 1024, layers[2], groups=max(
            1, groups//2), stride=2, norm_layer=norm_layer)
        self.layer4 = self._make_layer(
            block, 2048, layers[3], groups=groups, stride=2, norm_layer=norm_layer)
    ...
    def _make_layer(self, block, planes, blocks, groups, stride=1, norm_layer=None):
        ...
        layers.append(block(self.inplanes, planes, groups,
                            norm_layer=norm_layer, end_block=True, exclude_bn0=exclude_bn0))
